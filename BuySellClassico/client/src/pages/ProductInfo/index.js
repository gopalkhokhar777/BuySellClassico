...
import { GetAllBid, GetProductById } from "../../apicalls/product";
...
function ProductInfo() {
    ...
    const getData = async () => {
        try {
            ...
            if (response.success) {
                const responseBids = await GetAllBid({product:id})
                setProduct({...response.data,bids:responseBids.data});
            }
        } catch (error) {
            dispatch(SetLoader(false));
                    ...
                    <div className="flex gap-5">
                        {
                            product.images.map((image, index) => {
                                return (
                                    <img src={image}
                                        className={"w-20 h20 object-cover rounded-md cursor-pointer" +
                    ...
                    <div className="flex flex-col">
                  ...
                  
                      {
                        ...
                        product?.bids?.map((bid)=>{
                            return <div className="border border-gray-300 border-solid p-3 rounded mt-5">
